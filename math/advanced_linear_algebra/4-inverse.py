...
def adjugate(matrix):
    ...
    cofactor_matrix_copy = copy_matrix(matrix)

    for row in range(rows):
        for col in range(cols):
            matrix[row][col] = cofactor_matrix_copy[col][row]
...
def inverse(matrix):
    ...
    A matrix is singular if it's determinant is 0
    """
    rows = len(matrix)
    ...
    for row in range(rows):
        for col in range(cols):
            adjugate_matrix[row][col] = adjugate_matrix[row][col] / determinant
