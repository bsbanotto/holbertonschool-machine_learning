...
def convolve(images, kernels, padding='same', stride=(1, 1)):
    ...
    kernel: numpy.ndarray with shape (kh, kw, c, nc) containing the kernel for
    the convolution
    ...
    output_image = np.zeros((m, h, w, kn))
    ...
    for x in range(h):
        for y in range(w):
            for z in range(kn):
                i = x * sh
                j = y * sw
                hadamard_prod = np.multiply(images[:, i:i + kh, j:j + kw, :],
                                            kernels[:, :, :, z])
                output_image[:, x, y, z] = np.sum(hadamard_prod,
                                                  axis=(1, 2, 3))
    return output_image
